// Jenkins Pipeline - swift-main-ubuntu-hirsute
pipeline {
   agent { label 'arm64' }
   ...
   environment {
        ...
        DOCKER_IMAGE = 'swiftarm/ci-build:ubuntu_hirsute'
        CONTAINER = 'swift-main-dev-ubuntu-hirsute'
        OS = 'ubuntu'
        OS_VERSION = 'hirsute'
        ARCH = 'aarch64'
   ...
   stages {
      ...
      stage('Update Checkout') {
         steps {
            ...
            sh "./swift/utils/update-checkout --clone --tag ${SWIFT_SCHEME}"
         }
      }
      stage('Apply Patches') {
         steps {
